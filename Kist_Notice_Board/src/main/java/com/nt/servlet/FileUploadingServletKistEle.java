...
@WebServlet("/uploadurlkistele")
public class FileUploadingServletKistEle extends HttpServlet {
private static final String query="insert into upload_kist_ele values(kist_ele_seq.nextval,?,?,?)";
...
java.sql.Date sd=new java.sql.Date(ms);
			uplBean.setFolderstore("E:/store/photo/ele");
           ...
           System.out.println("FileUploadingServlet.doPost()");
			ps.setString(1, name);
			System.out.println("FileUploadingServlet.doPost()");
			ps.setString(2,sc.getInitParameter("UPLOAD_PLOC_ELE")+Fname1);
			System.out.println("FileUploadingServlet.doPost()");
			ps.setDate(3, sd);
			int count=0;
		...
		pw.println("<br>");
		pw.println("<a href='Uploadele.html'>Upload File</a>");
		pw.println("<br>");
